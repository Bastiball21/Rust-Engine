...
pub fn uci_loop() {
    ...
    let mut move_overhead = 10; // Default 10ms overhead
    ...
    loop {
        ...
        match command {
            "uci" => {
                println!("id name Aether 0.1.2");
                println!("id author Basti Dangca");
                println!("option name Hash type spin default 64 min 1 max 1024");
                println!("option name Threads type spin default 1 min 1 max 64");
                println!("option name SyzygyPath type string default <empty>");
                println!("option name Move Overhead type spin default 10 min 0 max 5000"); // New Option
                println!("uciok");
            },
            ...
            "go" => {
                ...
                // Pass move_overhead to parse_go
                let (tm, depth) = parse_go(game_state.side_to_move, &parts, move_overhead);
                
                let tt_ref: &'static TranspositionTable = unsafe { std::mem::transmute(&tt) };
                ...
                for i in 0..num_threads {
                    ...
                    search_threads.push(thread::spawn(move || {
                        search::search(&state_clone, tm_clone, tt_ref, stop_clone, depth, is_main, history_clone);
                    }));
                }
            },
            "stop" => {
            ...
            "setoption" => {
                if parts.len() > 4 && parts[1] == "name" {
                    if parts[2] == "Hash" && parts[3] == "value" {
                        ...
                        let path = parts[4];
                        println!("info string Syzygy path set to: {}", path);
                        // crate::search::init_syzygy(path); // Disabled for now to fix compilation
                    }
                }
            },
            "quit" => {
