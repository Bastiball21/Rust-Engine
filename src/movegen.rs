...
impl MoveGenerator {
    ...
    pub fn generate_moves(&mut self, state: &GameState) {
        ...
        if king.0 != 0 {
            ...
            // Castling logic (Basic check)
            if side == WHITE {
                if (state.castling_rights & 1) != 0 && !occupancy_all.get_bit(5) && !occupancy_all.get_bit(6) {
                    if !is_square_attacked(state, 4, BLACK) && !is_square_attacked(state, 5, BLACK) {
                         self.add_move(4, 6, None, false);
                    }
                }
                if (state.castling_rights & 2) != 0 && !occupancy_all.get_bit(1) && !occupancy_all.get_bit(2) && !occupancy_all.get_bit(3) {
                    if !is_square_attacked(state, 4, BLACK) && !is_square_attacked(state, 3, BLACK) {
                        self.add_move(4, 2, None, false);
                    }
                }
            } else {
                if (state.castling_rights & 4) != 0 && !occupancy_all.get_bit(61) && !occupancy_all.get_bit(62) {
                    if !is_square_attacked(state, 60, WHITE) && !is_square_attacked(state, 61, WHITE) {
                        self.add_move(60, 62, None, false);
                    }
                }
                if (state.castling_rights & 8) != 0 && !occupancy_all.get_bit(57) && !occupancy_all.get_bit(58) && !occupancy_all.get_bit(59) {
                    if !is_square_attacked(state, 60, WHITE) && !is_square_attacked(state, 59, WHITE) {
