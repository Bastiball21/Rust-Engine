// src/search.rs
use crate::state::{GameState, Move, WHITE, BLACK, K, k, Q, q, R, r, B, b, N, n, P, p, BOTH};
...
// Reduced LMP Table
const LMP_TABLE: [usize; 16] = [0, 2, 4, 7, 10, 15, 20, 28, 38, 50, 65, 80, 100, 120, 150, 200];
...
fn gives_check_fast(state: &GameState, mv: Move) -> bool {
    ...
    if let Some(p_promo) = mv.promotion { piece = if side == WHITE { p_promo } else { p_promo + 6 }; }
    ...
    if attacks.get_bit(enemy_king_sq) { return true; }

    let occ_no_source = Bitboard(state.occupancies[crate::state::BOTH].0 & !(1u64 << mv.source));
    let bishops = state.bitboards[if side == WHITE { B } else { b }] | state.bitboards[if side == WHITE { Q } else { q }];
    let rooks   = state.bitboards[if side == WHITE { R } else { r }] | state.bitboards[if side == WHITE { Q } else { q }];

    if (bitboard::get_bishop_attacks(enemy_king_sq, occ_no_source) & bishops).0 != 0 { return true; }
    if (bitboard::get_rook_attacks(enemy_king_sq, occ_no_source) & rooks).0 != 0 { return true; }
...
fn see(state: &GameState, mv: Move) -> i32 {
    ...
    let piece_vals = [100, 320, 330, 500, 900, 20000, 100, 320, 330, 500, 900, 20000, 0];
    ...
    if mv.is_capture && victim == 12 {
        gain[0] = 100;
        let ep_sq = if state.side_to_move == WHITE { to - 8 } else { to + 8 };
        occ.pop_bit(ep_sq);
    }
    ...
    loop {
        ...
        if (-gain[d - 1]).max(0) + piece_vals[piece] < 0 { break; }
...
fn get_attackers(state: &GameState, sq: u8, occ: Bitboard) -> Bitboard {
    ...
    if sq > 8 {
        if (sq % 8) > 0 && state.bitboards[0].get_bit(sq - 9) { attackers.set_bit(sq - 9); }
        if (sq % 8) < 7 && state.bitboards[0].get_bit(sq - 7) { attackers.set_bit(sq - 7); }
    }
    if sq < 56 {
        if (sq % 8) > 0 && state.bitboards[6].get_bit(sq + 7) { attackers.set_bit(sq + 7); }
        if (sq % 8) < 7 && state.bitboards[6].get_bit(sq + 9) { attackers.set_bit(sq + 9); }
    }
    ...
    let bishops = state.bitboards[2] | state.bitboards[8] | state.bitboards[4] | state.bitboards[10];
...
fn get_least_valuable_attacker(state: &GameState, attackers: Bitboard, side: usize, piece_type: &mut usize) -> u8 {
...
fn update_continuation_history(info: &mut SearchInfo, mv: Move, prev_move: Option<Move>, state: &GameState, bonus: i32) {
...
fn score_move(mv: Move, tt_move: Option<Move>, info: &SearchInfo, ply: usize, state: &GameState, prev_move: Option<Move>) -> i32 {
    if let Some(tm) = tt_move { if mv == tm { return 200000; } }
    ...
    if mv.promotion.is_some() { return 90000; }

    let mut score = 0;
    if ply < MAX_PLY {
        if let Some(k1) = info.killers[ply][0] { if mv == k1 { return 80000; } }
        if let Some(k2) = info.killers[ply][1] { if mv == k2 { return 79000; } }
    }

    if let Some(pm) = prev_move {
        let p_piece = get_piece_type_safe(state, pm.target);
        if let Some(cm) = info.counter_moves[p_piece][pm.target as usize] {
            if mv == cm { return 78000; }
        }
    }
...
fn get_piece_type_safe(state: &GameState, square: u8) -> usize {
    for piece in 0..12 { if state.bitboards[piece].get_bit(square) { return piece; } }
    12
}
...
fn get_victim_type(state: &GameState, square: u8) -> usize {
    ...
    for piece in start..=end { if state.bitboards[piece].get_bit(square) { return piece; } }
    12
}
...
fn quiescence(state: &GameState, mut alpha: i32, beta: i32, info: &mut SearchInfo, ply: usize) -> i32 {
    ...
    for i in 0..generator.list.count { scores[i] = score_move(generator.list.moves[i], None, info, ply, state, None); }
        ...
        if !in_check {
            ...
            if mv.is_capture {
                ...
                if see_val < 0 {
                    ...
                    if !gives_check_fast(state, mv) { continue; }
                }
            }
        }

        if !info.tt.is_pseudo_legal(state, mv) { continue; }
...
pub fn search(state: &GameState, tm: TimeManager, tt: &TranspositionTable, stop_signal: Arc<AtomicBool>, max_depth: u8, main_thread: bool, history: Vec<u64>) {
    ...
    for depth in 1..=max_depth {
        ...
        loop {
            ...
            score = negamax(state, depth, alpha, beta, &mut info, 0, true, &mut path, None, None, None);
        ...
        if let Some(bm) = final_move {
            ...
            for &lm in &legal_moves {
                if bm.source == lm.source && bm.target == lm.target && bm.promotion == lm.promotion {
                    found = true; final_move = Some(lm); break;
                }
            }
            if !found { final_move = None; }
        }

        if final_move.is_none() && !legal_moves.is_empty() { final_move = Some(legal_moves[0]); }

        if let Some(bm) = final_move {
            print!("bestmove {}{}", square_to_coord(bm.source), square_to_coord(bm.target));
            if let Some(promo) = bm.promotion {
                use crate::state::{q, r, b, n};
                let char = match promo { 4|10=>'q', 3|9=>'r', 2|8=>'b', 1|7=>'n', _=>' ' };
                if char != ' ' { print!("{}", char); }
            }
...
fn negamax(state: &GameState, depth: u8, mut alpha: i32, beta: i32,
           info: &mut SearchInfo, ply: usize, is_pv: bool, path: &mut Vec<u64>,
           prev_move: Option<Move>, prev_prev_move: Option<Move>, excluded_move: Option<Move>) -> i32 {
    ...
    let in_check = is_in_check(state);
    let new_depth = if in_check { depth.saturating_add(1) } else { depth };

    if new_depth == 0 { return quiescence(state, alpha, beta, info, ply); }
    ...
    if let Some((score, d, flag, mv)) = info.tt.probe_data(state.hash) {
        ...
        if ply > 0 && excluded_move.is_none() && d >= new_depth {
            ...
            if flag == FLAG_ALPHA && score <= alpha { return alpha; }
            if flag == FLAG_BETA && score >= beta { return beta; }
        }
    }
    ...
    if !is_pv && !in_check && excluded_move.is_none()
       && new_depth < 7 && static_eval - (80 * new_depth as i32) >= beta {
        return static_eval;
    }

    if new_depth >= 3 && ply > 0 && !in_check && !is_pv && excluded_move.is_none() && static_eval >= beta {
        use crate::state::{N, B, R, Q, n, b, r, q};
        ...
        if has_pieces {
            ...
            let score = -negamax(&null_state, reduced_depth, -beta, -beta + 1, info, ply + 1, false, path, None, None, None);
            if info.stopped { return 0; }
    ...
    if is_pv && tt_move.is_none() && new_depth > 4 {
        ...
        negamax(state, iid_depth, alpha, beta, info, ply, is_pv, path, prev_move, prev_prev_move, None);
    ...
    let mut extension = 0;
    if ply > 0 && new_depth >= 8 && tt_move.is_some() && excluded_move.is_none()
       ...
       && tt_score.abs() < MATE_SCORE
    {
        ...
        let score = negamax(state, reduced_depth, singular_beta - 1, singular_beta, info, ply, false, path, prev_move, prev_prev_move, tt_move);
    ...
    for i in 0..generator.list.count {
        ...
        if !is_pv && !in_check && new_depth <= 8 && is_quiet {
            if quiets_checked >= LMP_TABLE[new_depth as usize] { continue; }
        }
        ...
        let our_king = if our_side == WHITE { K } else { k };
        let king_sq = next_state.bitboards[our_king].get_lsb_index() as u8;
        if movegen::is_square_attacked(&next_state, king_sq, next_state.side_to_move) { continue; }

        info.tt.prefetch(next_state.hash);
        moves_searched += 1;
        if is_quiet { quiets_checked += 1; }

        let mut score;
        if moves_searched == 1 {
            let extended_depth = new_depth.saturating_add(extension);
            score = -negamax(&next_state, extended_depth - 1, -beta, -alpha, info, ply + 1, true, path, Some(mv), prev_move, None);
        } else {
            ...
            if new_depth >= 3 && moves_searched > 1 && is_quiet && !gives_check && !in_check {
                ...
                if improving { reduction = reduction.saturating_sub(1); }
            ...
            score = -negamax(&next_state, d, -alpha - 1, -alpha, info, ply + 1, false, path, Some(mv), prev_move, None);

            if score > alpha && reduction > 0 {
                score = -negamax(&next_state, new_depth - 1, -alpha - 1, -alpha, info, ply + 1, false, path, Some(mv), prev_move, None);
            }
            if score > alpha && score < beta {
                score = -negamax(&next_state, new_depth - 1, -beta, -alpha, info, ply + 1, true, path, Some(mv), prev_move, None);
            }
        }
    ...
    if excluded_move.is_none() {
        info.tt.store(state.hash, max_score, best_move, new_depth, flag);
    }
    max_score
}
...
fn moves_gives_check(state: &GameState, mv: Move) -> bool {
    if gives_check_fast(state, mv) { return true; }
    let next_state = state.make_move(mv);
