...
pub fn get_scale_factor(state: &GameState, raw_score: i32) -> i32 {
    let strong_side = if raw_score > 0 {
        state.side_to_move
    } else {
        1 - state.side_to_move
    };

    let w_mat_no_pawn = state.bitboards[N].count_bits()
        + state.bitboards[B].count_bits()
        + state.bitboards[R].count_bits()
        + state.bitboards[Q].count_bits();
    let b_mat_no_pawn = state.bitboards[n].count_bits()
        + state.bitboards[b].count_bits()
        + state.bitboards[r].count_bits()
        + state.bitboards[q].count_bits();
    ...
    if w_mat_no_pawn == 1
        && b_mat_no_pawn == 1
        && state.bitboards[B].count_bits() == 1
        && state.bitboards[b].count_bits() == 1
    {
    ...
    let (mat_strong, mat_weak) = if strong_side == WHITE {
        (w_mat_no_pawn, b_mat_no_pawn)
    } else {
        (b_mat_no_pawn, w_mat_no_pawn)
    };

    if mat_weak == 0 && mat_strong == 1 && pawns_count > 0 {
        // Identify if strong side has only a Bishop
        let bishop_bb = if strong_side == WHITE {
            state.bitboards[B]
        } else {
            state.bitboards[b]
        };
        ...
        if bishop_bb.count_bits() == 1 {
            let pawn_bb = if strong_side == WHITE {
                state.bitboards[P]
            } else {
                state.bitboards[p]
            };
